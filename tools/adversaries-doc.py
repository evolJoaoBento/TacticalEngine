...
out.append('- **a rule about a third creature** - "disadvantage on attacks against targets other')
out.append('  than the Swarm", "attacks against a creature this one stands beside have advantage". A')
out.append('  passive can move the dice for its holder or against them (`advantage`, with `against`)')
out.append('  and that is two parties; a third is not something a modifier can name.')
out.append('- **a token nobody else can count** - a token on the stat block runs. The store is keyed')
out.append('  by creature and card, so a passive keeps a count on itself: Slow is one `spotlighted`')
out.append('  reaction that branches on its own tokens, placing one on the turn it gathers and')
out.append('  spending it on the turn it acts, with `endSpotlight` paying for the wait. What stays')
out.append('  text is a token another creature carries and other cards read - the bramble tokens a')
out.append('  Swarm hands out, which every one of its features asks a different question about -')
out.append('  and ground that stays dangerous after the feature ends.')
