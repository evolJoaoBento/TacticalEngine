...
out.append("  A trigger about what another creature did is no longer one of them: a blow that has")
out.append("  landed and not yet been counted now stops for the room. `rollingDamage` is raised on")
out.append("  the one swinging and `allyRollingDamage` on everyone else still standing, with the")
out.append("  one being hit bound as the target, and `boostDamage` is what they add - a flat")
out.append("  number, dice, or the reactor's own printed attack. So the Turret fires into an")
out.append("  ally's hit and the Demon of Jealousy cannot bear to be outdone. What stays text")
out.append("  there is the amount Reaper wants, a bonus equal to the Demon's own marked Hit")
out.append("  Points, which is a number read off a pool; and a Shark that answers somebody")
out.append("  else's hit by taking a turn of its own.")
...
out.append('  dice and type. An amount can also be read off a pool - a bonus equal to the')
out.append('  number of Hit Points a creature has marked - with the same three words the')
out.append('  `pool` condition asks with, so a gate and an amount read one number. What stays')
out.append('  text is every other number: one the block keeps rather than a pool (handfuls of')
out.append('  gold), one spent per target rather than in total ("lose a number of Hope equal')
out.append('  to the HP they marked"), one that says how many dice to roll rather than how')
out.append('  much to do, one that decides how many creatures a spotlight reaches, and a Fear')
out.append('  cost that is a number rather than a price - the vocabulary gains Fear and never')
out.append('  spends it.')
out.append('- **a condition the SRD prints once** — Entranced, Cursed, Deathlock and the')
out.append('  rest: written into that one block rather than into the rules. A few have since been')
out.append('  written down as a name and nothing more - Poisoned is one - where what the name does')
out.append("  is the block's own words and the table's to play.")
...
out.append('  bound. A band can be swapped in as well as dice: "they deal Severe damage instead of')
out.append('  their standard damage" names the band outright, either from a passive on the block or')
out.append('  from the moment a blow stops to be counted, and the thresholds have nothing to say')
out.append('  about it - the Armor Slots still do.')
out.append('  What stays text is the asking: standing above somebody, a roll that had')
