...
- The project's own work is licensed **CC BY-NC 4.0** (`LICENSE`, `NOTICE.md`), which is what a
  public repository needs to grant anybody anything: share and adapt for anything not commercial,
  with credit. It licenses the code that implements the rules, not the rules. It was
  AGPL-3.0-or-later until 24 September 2026, and copies from before then stay so.
