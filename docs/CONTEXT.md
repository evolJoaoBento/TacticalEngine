...
- The project's own work is licensed **AGPL-3.0-or-later** (`LICENSE`, `NOTICE.md`), which is what
  a public repository needs to grant anybody anything. It licenses the code that implements the
  rules, not the rules, and §13 means a modified copy served to other people owes them its source.
