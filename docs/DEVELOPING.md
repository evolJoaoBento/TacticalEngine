...
5. `src/game/demo-scene.ts` (5266 lines) — where the engine is actually wired into a playable thing;
...
| Path | What it is |
|---|---|
| `demo-scene.ts` | `DemoScene` and the functions over it (5266 lines, being taken apart — `BACKLOG.md` §2): `buildDemoScene`, `buildProjectScene`, `worldOptions`, `attackWithSelected`, `playGmTurn`, `endTurn`, `defenseChoices`, `applyDefenseChoice`, `useSelectedOn`, `answerPending`, `travelTo`. |
