...
5. `src/game/demo-scene.ts` (4437 lines) — where the engine is actually wired into a playable thing;
   `src/game/log.ts` is what the player reads of it.
6. `src/main.ts` (2562 lines) — the boot path. It owns the board, play and global keys
...
| `content/equipment/catalogue.ts` | The equipment the engine ships: `catalogue.json` (324 weapons, 69 armours, 633 items, written by `tools/loot-cards.mjs` from the user's loot-card export and parsed through the engine's own schemas at load), `itemOf(project, id)` and `itemsFor(project)` - the project's items laid over the catalogue's by id. Every item lookup in the game and the editor goes through these; `DEMO_CHARACTERS` carries the catalogue's weapons and armours beside the starter pack's. The generator's own decisions, none of them the cards': worth by tier (weapon 10/25/60/150, a secondary six tenths of that, armour 15/35/80/200) or by loot roll (consumables 2 + roll/6, items 5 + roll/2), a weapon's slot from its damage type, the six health and stamina potions scripted as `heal` / `clearStress` with the card's dice, and a Spellcast weapon (`trait: 'spellcast'`) swung with the wielder's spellcast trait (Knowledge for one who casts with nothing; `wieldedTrait` in `sheet.ts`). The pictures: `public/equipment/*.webp`, on Hugging Face beside the models, named in `equipment.lock.json` (`tools/lock-equipment.mjs` writes it after an upload) and fetched by `npm run models`. |
| `content/equipment/features.ts` | `gearEffects(features)`: a weapon's or armour's feature read clause by clause for the numbers it plainly states - "+1 to attack rolls", "−1 to Evasion", "+2 to Armor Score", "−1 to Finesse", "+1 to all character traits and Evasion" - into `abilityModifierSchema` modifiers and trait changes. `deriveCharacter` adds them for the primary, the secondary and the armour, so they fold into the sheet as a passive card's do and an attack reads Reliable through `rollBonus('attackRoll')`. 68 cards' features are wholly numbers and 13 more partly; anything else a feature says is its text. |
| `scene/party.ts` | Selection, order (`members`, `arrange`), the leader trail a group walks down (`remember`, `downTheTrail`), groups (`groupOf`, `link`, `unlink` - the followers of a walk are the walker's group), held members (`hold`, `release`, `isHeld` - no orders, not followed, still grouped), follower movement, and what a walk costs: `planWalk` finds the way without a budget and measures the *line* (`lineCost` in `grid/walk.ts`); `short: true` cuts it where the allowance runs out (`distanceWithin`, `splitLine`) and returns the rest as `beyond`; `covered` is the lit ground. |
| `scene/interact.ts`, `scene/triggers.ts` | Interactables and trigger cells. A trigger whose encounter placed creatures that are all on nobody's side - friendly, or talked round - wakes nothing. |
...
| `dialogue/schema.ts`, `dialogue.ts`, `layout.ts` | Conversation graphs and their editor layout. A node of `kind: 'consequence'` is never shown: `afterEnter` runs its `onEnter` and walks to its `goto` or ends. `DialogueRunner` takes `ScriptRunnerOptions` (`targets`, `subject`) and hands them to every script inside, which is how a conversation knows whom it is with. |
...
| `render/click-ripple.ts` | `ClickRipples`: the ground answering a click to walk - two rings spreading and fading from the spot, a beat apart, and a dot that pops - or, for a click that walks nobody, a small red ring that shakes. `SceneView.ripple(spot, ok)` puts one down; six at most, the newest kept. |
...
| `tools/add-portal-glow.mjs` | `node tools/add-portal-glow.mjs <in.glb> <out.glb> <cx> <cy> <rx> <ry> [<depth>]`: a glowing, turning vortex over a portal's opening, inside the .glb - two emissive discs (KHR_materials_emissive_strength), one per face, each spinning in a frame that holds the opening's oval, and an eight-second clip, `portal-glow`, that the engine loops as it does any model's first clip. Node rotation and scale only, since three reads KHR_animation_pointer only through a plugin. The numbers are the opening in the model's first node's own frame; `portal-prop` took (0.011, -0.07), 0.27 by 0.36. |
| `tools/add-top-rune.mjs` | `node tools/add-top-rune.mjs <in.glb> <out.glb> [<glyph>] [<share>] [--preview <png>]`: a glowing glyph carved into the top of a model, inside the .glb. The top is found from the first node's own vertices (everything within 6 mm of the highest); a decal a hair above it carries the glyph (π unless said) with a dark groove round the stroke, so it reads as cut rather than painted, and an emissive glow (KHR_materials_emissive_strength 1.6). `pillar-prop` took the defaults. |
| `tools/add-flame-glow.mjs` | `node tools/add-flame-glow.mjs <in.glb> <out.glb> [<strength>] [--mask <preview.png>]`: makes a one-texture model's fire glow, inside the .glb. Every pixel of its colour texture is weighed by how plainly it is fire - red-orange to yellow, saturated, bright, with soft edges on each - and the weighed colour becomes the material's emissive map, at KHR_materials_emissive_strength 3 unless said. Wood and iron weigh nothing. Both torches took the default; `--mask` writes the map as a PNG to check what counted as fire. |
| `tools/recolour-model.mjs` | Node and Playwright's Chromium, no Blender: `node tools/recolour-model.mjs <in.glb> <out.glb> <saturation|green> <0..1>` (or `match <reference.glb>`) changes the colour of a model's base-colour textures only (a normal or roughness map is data, and is left alone) and carries every other byte across, whatever format the images are in - WebP behind `EXT_texture_webp` included. `saturation` takes colour out towards the grey of the same brightness; `green` turns browns towards the texture's own grass, hue only, keeping each pixel's saturation and lightness; `match <reference.glb>` measures the browns of both textures and moves the first onto the second - saturation and lightness scaled, hue turned - so two tiles that meet agree; `match-all <reference.glb>...` does it over every pixel against the average of several references; `lightness <factor>` scales lightness alone. The crate, barrel, cart, door, chest, standing torch and training dummy are at saturation 0.6, the grass ground at green 0.85, and the dirt ground is matched to the dirt in `grass-dirt-ground` and then taken to 70% of that saturation (`saturation 0.7`), which is what looked right beside it on the board; the stone wall is `match-all` to the stone block and stairs and then `lightness 0.88`, its stone faces having come up brighter than theirs; the files before each change are in `public/models/heavy/`. |
| `tools/fetch-models.mjs` | `npm run models`: downloads every model `models.lock.json` names from its Hugging Face repo into `public/models/`, checking size and SHA-256, and leaves a file already right alone; `-- --check` only reports. The models are not in git (`.gitignore`, guarded by `tests/unit/models-lock.test.ts`). |
| `tools/lock-models.mjs` | `node tools/lock-models.mjs <owner/repo> <revision>` writes `models.lock.json` from what is in `public/models/`. Adding or changing a model: put it in the folder, `shrink-textures.mjs` it, `hf upload JunjiBento/tactical-engine-models public/models . --include "*.glb" --exclude "heavy/*"`, then lock to the revision the upload printed and commit the list. |
| `tools/shrink-textures.mjs` | Node and Playwright's Chromium: `node tools/shrink-textures.mjs <in.glb> <out.glb> [<colour px> <other px>]` re-encodes a model's maps as WebP (EXT_texture_webp) at the pipeline's sizes - colour 2048, everything else 1024 - never larger, and only where that is lighter. Nothing else in the file is touched, so a rig, a clip, a second mesh or the portal's glow come through, where `lighten-model.py` re-exports the whole file and wants one mesh. The thirteen heaviest models went through it, 143 MB to 20; the files before are in `public/models/heavy/*.before-shrink.glb`. Run it on any model that arrives with PNG or top-quality JPEG maps before it is uploaded. |
...
| `render/camera.ts` hold | `OrbitCamera.hold(target, distance)` / `release()` / `held`: a conversation holds the camera on whoever is talked to - pan, zoom and follow do nothing while held, `orbit` still turns - and release puts back the target and distance it had. `CameraFocus.tick` holds and releases it off `talking` (`game/ui/play-views.ts` `talkingTo`: the creature a conversation is `with`, else the thing used). |
| `game/ui/GearBinder.tsx` | The gear pages: `useGearPages` returns the plastic film of labelled sleeves for the left leaf, the pack in sleeves for the right, the drag's ghost and the reading lightbox, because a drag crosses both leaves. The drag is pointer events (as `PartyHud`'s), dropped by `document.elementFromPoint` on `[data-slot]` or the pack; a press that never moves is a click and holds the card up to read. `GearFace` is a card: its picture (`equipment-art.ts`, only for a file `equipment.lock.json` names, falling back on a load error) or drawn from its numbers. `LoadoutPanel` lays both films over its leaves, never replacing them, so the pages keep their measured size, and runs the turn to and from them (`turnGear`, `GEAR_TURN_MS`): the card pages (`.deck-leafstack`, wrapping the toolbar and the pages, with the Equipment divider's tab on its edge) turn up off the right for the first half, the divider's back (`.gear-divider`, holding the left film and the Back tab) comes down on the left for the second, and the pack is under the stack. The stack is `visibility: hidden`, never unmounted, while the divider is open, so the pages keep their measured size. The binder itself has no Close button. The shop window draws its lines with `GearFace` too. |
| `game/ui/Conversation.tsx` | A conversation along the bottom where the cards are: `ActionBar` shows it in place of the Jump and Rest keys and the hand while `talking` (`talkingView`), and the play panel no longer draws one. A `TalkingView` with `held` set - a shop it opened is still open (`shopOpen`) - shows why and shuts every reply; `answerPending` refuses a conversation's answer then too, so the driver cannot answer past the shop either. |
| `render/faction-outline.ts` | The line round every creature: `DEFAULT_FACTION_COLORS` (party white, adversary red, neutral - friendly - green, `truce` yellow), `outlineSide` (which of them a creature is drawn in: `truce` while an End a fight stands it down, else its faction), `litOutlines` (full strength under the pointer, blue for the selected, dim for the rest). The view rebuilds a token whose `outlineSide` changed, since a rim is drawn once. |
...
| `render/tile-models.ts` | The ground drawn as models rather than as coloured quads. A terrain type that names a `model` stands one on every tile of that type, at the tile's own height; `redrawTileModels` takes the last lot down and puts the new lot up, which is how a room change or a late-arriving asset redraws. The terrain mesh is still built underneath - a tile model is a look, and the grid is what a raycast hits and what a walk costs, so a file that fails to load leaves the room playable. |
| `render/authoring-marks.ts` | What the editor draws for things with no body of their own: a party start nobody fills (the legacy blue ring, with a pawn to take hold of) and an object with no model (a gold ring and a gem). `setAuthoring` draws them; play never does. A start the party fills is drawn as that character's model, named `spawn:<i>` all the same, so carrying finds it. |
...
| `render/procedural/` | `spec.ts`, `registry.ts` (falls back to a placeholder rather than throwing; `missing()` lists unresolved ids), `build.ts`, `library/{heroes,monsters,props}.ts`. The props library is empty: all fourteen were retired for imported models (`scene/retired-models.ts`). |
...
| `movement.ts` | Walking: `reachableTiles`, `walkTheMove`, `underPressureTiles`, `closeToStrike`, `previewStrike`, `previewWalk`, `arrive`, `startEncounter` (the fight starts when the walkers reach the trigger), `beginScriptedFights` (a script's Start a fight, read off the journal by `react`: begins the fight when none runs and somebody in the room is hostile or stood down). `startEncounter` turns every creature marked `truce` hostile again, so every way a fight begins does. `moveSelectedTo` stays in `demo-scene.ts`: the click. |
...
| `interaction.ts` | Creatures that talk (`AdversaryPlacement.interaction`). `talksTo`/`talkTo`: a friendly creature is talked to rather than struck - `attackWithSelected` hands it here - with the creature bound as the conversation's `target` (`PendingScript.with`, which `settle` passes to the `DialogueRunner`). `playTurnings`, run by `settleFight` after the wounds are answered: a threshold creature a blow left at or under its share turns friendly, `interacted` is set on it (saved), and its conversation opens with the fight holding; the conversation's `onDone` settles the fight again. `reactToAttitudes`, run by `react`: a creature turned hostile with no fight running starts its encounter's fight, a fresh runner if the last one ended. `fightOverLine` says whether a fight ended with the last one down or the last one talked round. `stopScriptedFights`: a script's End a fight ends the runner with outcome `stopped` and stands every enemy down (`truce`, saved); `resumeOnBlow` begins the fight again at a blow struck at one. |
| `gear.ts` | What the loadout's gear pages show: `GEAR_SLOTS` (primary, secondary, armour, with their labels), `gearView(demo, characterId)` - the three places with the card worn in each, and the pack as `CarriedCard`s in the order it was filled - and `gearCard(demo, itemId)`, any item as a card: a weapon's trait, range, damage and hands, armour's thresholds, score and tier, its feature, and its picture's `card` file when it has one. Starter gear with no picture is drawn from the same numbers. `equip.ts` has `unequipItem` beside `equipItem`, and counts hands: a two-handed primary returns the secondary to the pack, and a secondary is refused under one. |
| `talks.ts` | A conversation is its speaker's. `PendingDialogue.by` is the member who opened it (`scenario.actorId` when `settle` opens it). `syncTalks` - run by `main.ts` on every `refreshPlay` - moves the conversation on `demo.pending` aside when somebody else is selected (out of a fight, no GM turn stopped in it), stashing any shop it had open, and holds the speaker (`Party.hold`: `canCommand` false, left out of `followersOf`, still in the group); it puts it back on `pending`, shop and all, when they are selected again with nothing else pending. A fight (with a word in the log), the room entered afresh - travel or a load, which `install` gives a new `Party` - or the speaker falling ends what is set aside; each set-aside talk keeps the `Party` it came from, so that is an identity check. The HUD keeps Level Up shut for a held member. `talkingAside` feeds the HUD's *In conversation* mark, `saveBlockedBy` and `rest`. Everything that refuses while `pending` is set is untouched - setting aside is moving the prompt off that slot and back. |
| `arrival.ts` | An interaction that waits for its walk. `approachThenUse` (a thing) and `talkTo` (somebody) walk the selected member there at once in the document and, when the walk is `animated`, leave what it was for on `demo.approaching`; the frame loop runs `arrived` once `view.glidingCount` is 0 (after an ambush's `arrive`, which drops it). `cancelApproach` is a right-click on the way, or any new order in `clickAt`. Headless nothing waits. The walk up is `walkSelected` (`movement.ts`), which out of a fight brings the followers along (`followAlong`) as `walkTheMove` does - for a talk, a use and a swing's walk alike. The driver's `approach` and `attack` run `arrived` at once, not waiting for the walk to be drawn. |
| `shop.ts` | Buying. `shopOf` finds a seller's stock - a prop's Shop function or a creature's `interaction.shop` - `shopContents` what is left (a limited line's sales counted on the seller's saved `interactable(id).data`), `buyFrom` pays from the pack or refuses with a word in the log, `purse`; `sellables`/`sellTo` buy what the party carries at `offerFor` - half the seller's own price for one of its lines (a limited line going back on the shelf), else half the item's `value`, always through `buyBackPrice` at the shop's `buysAt` share (half when it says nothing; capped at the whole worth, so no line can be bought and sold back for a profit; rounded down, never nothing); no value, no sale. `prop-use.ts` opens the window on a `shop` journal entry (from `openShop`) and hands a shop's lines and Take to here, so the window is the container's with a price on each line and the driver's `container`/`take` handles buy too. A shop's window is drawn in the middle of the screen over a dimmed `shop-backdrop` (z 50: over the HUD, under the loadout binder's 60; `hud.css` lifts the Loadout book to 51 while it is there), a chest's stays in the panel stack (`ContainerWindow` in `PlayPanel.tsx`). Its ✕ or a click on the backdrop closes it; Esc does not - neither the shop nor the dialogue is in `ESCAPE_IS_TAKEN`, so Esc opens the settings (z 70) over both. |
| `cues.ts` | `cuesFrom`: what a script's journal means to the countdowns. Out of `demo-scene.ts` to give it room. |
...
| `ui/` | `ActionBar`, `PartyHud`, `PlayPanel`, `LoadoutPanel`, `LevelUpPanel`, `RestPanel` (Preact). A check a player is asked for is thrown on `RollStage`: the room blurs out behind a card naming the roll, and it holds the dice until they are read — every other Duality roll goes to `DiceTray` behind it, which waits while the card is up. `Die` draws one die and `d12.ts` is the solid itself, twelve numbered pentagons turned and flattened, so a die tumbles in three dimensions with no art to ship. `hud.css` is the play overlay's look -- the deck browser's palette (`cards.css`) named once as `--play-*` tokens -- and the HUD, bar, panel and rest wear its classes rather than inline styles. `CardFace` draws a card; `CardArtImport` is the per-browser art picker under it, and `CardPreview` is the two together, which `main.ts` hands to the editor's Cards panel as a render prop -- the editor never imports the game. |
...
| `ui/StockEditor.tsx` | What a shop sells, for a creature's interaction and a Shop prop alike: currency, and a line per item with price and an optional count. |
| `creature-edits.ts` | `setCreatureInteraction`: a placed creature's interaction set or cleared whole, one undo step. Out of `session.ts` for the same reason. |
...
| `model-memory.ts` | The models imported in this browser, kept in IndexedDB and laid back under the project as the editor opens. `missingFrom` is the rule — a project's own declaration wins the id — and the store is injected, so it is tested with a Map. Because every remembered model is laid back under every project, a save runs the project through `withoutUnusedEmbedded` (`asset-edits.ts`) first: an embedded model nothing names (`scene/model-references.ts`: every string in the project outside its model list, which errs wide on purpose) stays in the browser and out of the file. |
...
| `ui/EffectList.tsx`, `ui/effect-list-parts.ts` | Editing a list of effects, recursively. `ADDABLE` is the list of kinds it can build; the pure helpers (`parseArgs`, `ADVANCES`, `describe`) are in the parts file. |
...
| `ui/SheetEditor.tsx` | One character's sheet as a form: the Party panel's right-hand side, and the Inspector's side pane when a party start is in hand. One component, so the two never disagree. |
...
`close-<panel>` (a back arrow now, labelled "Back"; the testid kept its name). Row ids interpolate the content id. There are 124 of them across the panels. Prefer
...
- **Name a creature with `nameOf` (`game/log.ts`), never its stat block.** A placement's own name
  rides on the entity (`EntityState.name`) and comes first; `adversaryDef(...).name` is what it is,
  not what it is called. At the head of a sentence use `theNameOf` - "The Hollow Knight's Greatsword",
  but "Captain Vey's", never "The Captain Vey's". Tests that name a creature use the same two.

- **A creature's side can change mid-fight.** `faction` is read as it goes everywhere a fight counts
  (`entitiesOf`), so `SceneState.setAttitude` is all it takes - but anything that took a list of
  adversaries earlier and walks it later must check again: the GM turn's queue skips whoever is no
  longer `adversary`, and the view rebuilds a token whose faction changed, since its rim is drawn
  once. A creature's `faction` is saved with the room, and so is `interacted`.
