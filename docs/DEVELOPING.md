...
| `render/procedural/` | `spec.ts`, `registry.ts` (falls back to a placeholder rather than throwing; `missing()` lists unresolved ids), `build.ts`, `library/{heroes,monsters,props}.ts`. The props library is down to the pillar: the other thirteen were retired for imported models (`scene/retired-models.ts`). |
