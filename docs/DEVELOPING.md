...
| `render/scene-view.ts` | `SceneView` — engine state to three.js objects, one way (1469 lines). |
...
| `render/spotlight.ts` | `Spotlight`: the white rim on whatever the pointer is over, drawn through what stands in front of it. Inverted hulls on the outline layer, so the editor never draws them and no raycast finds them. |
